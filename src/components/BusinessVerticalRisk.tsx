...
const BusinessVerticalRisk = ({
  ...
  <path d="M12 2L6 14h12z" fill={colorClass} />
        ...
        <div className="group relative ml-4 cursor-pointer">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 mt-2 w-max max-w-xs p-3 text-xs bg-gray-700 text-white rounded shadow-lg hidden group-hover:block z-50 whitespace-pre-wrap">
            ...
            <div><strong>Business Potential</strong>: Adjusted Projected Value for the Selected Period (based on Ongoing Initiatives):<br />
              ...
              factor_based_on_time_left: Proportion of time remaining in the current year<br />
              ...
              {['current', 'currentStar', 'industryAverage', 'cutoff'].map((key, i) => (
                isValidNumber(vertical[key]) && (
                  ...
                  >
                    <TrianglePointer
                      value={vertical[key]}
                      label={getPointerLabel(key)}
                      formatFunc={formatValue}
                      colorClass={getPointerColor(key)}
                    />
                  </div>
                )
              ))}
   ...
   <div className="w-1/5 text-center flex items-center justify-center">
        {formatValue(vertical.estimated_ebitda)}
      </div>
            {/* Status */}
